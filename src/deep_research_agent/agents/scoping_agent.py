...
class ScopingAgent:
    def __init__(self):
        ...
        self.clarify_with_user_prompt = get_prompt_template(
            os.path.join(
                ROOT_DIR,
                "src/deep_research_agent/prompts/clarify_with_user_instruction.jinja",
            )
        )
        self.write_research_breif_prompt = get_prompt_template(
            os.path.join(
                ROOT_DIR,
    ...
    async def clarify_with_user(
        ...
        response = structured_output_model.invoke(
            [
                SystemMessage(
                    content=self.clarify_with_user_prompt.render(
    ...
    async def write_research_brief(self, state: AgentState):
        ...
        response = structured_output_model.invoke(
            [
                SystemMessage(
                    content=self.write_research_breif_prompt.render(
